...
int NBClient::connect()
{
  ...
  if (_synch) {
    ...
    while (ready() == 0)
    {
      if(_connectTimeout && ((millis() - start_millis) > _connectTimeout)) return 0;
    }
  } else if (ready() == 0) {
    return 0;
  }
...
void NBClient::set_connect_timeout(unsigned long ms)
{
  _connectTimeout = ms;
}
...
void NBClient::set_write_timeout(unsigned long ms)
{
    _writeTimeout = ms;
}
...
size_t NBClient::write(const uint8_t* buf, size_t size)
{
  ...
  if (_writeSync) {
    while (ready() == 0)
    {
      if(_writeTimeout && (millis() - start_millis) >= _writeTimeout) return 0;
    }
  } else if (ready() == 0) {
    return 0;
  }
